...
# 安装readxl（需要系统依赖libxls-dev）
if (!requireNamespace("readxl", quietly = TRUE)) {
    cat("\n安装 readxl...\n")
    tryCatch({
        # 首先尝试安装依赖包
        if (!requireNamespace("cellranger", quietly = TRUE)) {
            install.packages("cellranger", lib = user_lib, dependencies = FALSE)
        }
        if (!requireNamespace("tibble", quietly = TRUE)) {
            install.packages("tibble", lib = user_lib, dependencies = FALSE)
        }
        # 安装readxl
        ...
        tryCatch({
            ...
            # 再次尝试安装readxl
            install.packages("readxl", lib = user_lib, dependencies = TRUE, repos = c(CRAN = "https://cloud.r-project.org/"))
            if (requireNamespace("readxl", quietly = TRUE)) {
                cat("  ✓ readxl 安装成功（第二次尝试）\n")
            } else {
                stop("readxl仍然无法加载")
            }
        }, error = function(e2) {
            cat("  ✗ readxl 安装仍然失败:", conditionMessage(e2), "\n")
            cat("  请确保系统已安装 libxls-dev 或 libxls 库\n")
            cat("  在Ubuntu/Debian上运行: sudo apt-get install libxls-dev\n")
            cat("  在Windows上，可能需要安装Rtools\n")
        })
    })
} else {
    cat("✓ readxl 已安装\n")
