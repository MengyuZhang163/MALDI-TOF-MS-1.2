...
def generate_demo_data(n_samples=5, n_features=100):
    """生成演示数据"""
    np.random.seed(42)
    
    mz_values = np.sort(np.random.randint(1000, 10000, n_features))
    intensity = np.random.exponential(scale=100, size=(n_samples, n_features))
    
    col_names = [f"mz_{mz}" for mz in mz_values]
    row_names = [f"Sample_{i+1}" for i in range(n_samples)]
    
    df = pd.DataFrame(intensity, columns=col_names, index=row_names)
    df.insert(0, '行名', row_names)
...
with tab2:
    ...
    if process_btn or demo_btn:
        with st.spinner("正在处理数据..."):
            ...
            # 生成演示数据
            n_samples = len(st.session_state.uploaded_files_info['train_txt']) if process_btn else 5
            st.session_state.demo_data = generate_demo_data(n_samples=max(n_samples, 3))
    ...
    if st.session_state.demo_data:
        ...
        with col1:
            show_rows = st.number_input(
                "显示行数",
                min_value=1,
                max_value=len(st.session_state.demo_data['train']),
                value=min(5, len(st.session_state.demo_data['train']))
            )
        ...
        display_df = st.session_state.demo_data['train'].head(show_rows)
        ...
        with st.expander("📊 查看详细统计"):
            st.write("**基本统计信息:**")
            stats_df = st.session_state.demo_data['train'].iloc[:, 1:].describe().T
